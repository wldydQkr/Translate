...
final class TranslateViewController: UIViewController {
    ...
    private lazy var targetLanguageButton: UIButton = {
        let button = UIButton()
        button.setTitle(translateManager.targetLanuguage.title, for: .normal)
        button.titleLabel?.font = .systemFont(ofSize: 15.0, weight: .semibold)
        button.setTitleColor(.label, for: .normal)
        button.backgroundColor = .systemBackground
        button.layer.cornerRadius = 9.0
    ...
    @objc func didTapBookmarkButton() {
        ...
        let newBookmark = Bookmark(
            ...
            translatedLanguage: translateManager.targetLanuguage,
    ...
    private lazy var sourceLabel: UILabel = {
        let label = UILabel()
        label.text = NSLocalizedString("Enter_text", comment: "텍스트를 입력해주세요.")
    ...
    func didTapLanguageButton(type: Type) {
        ...
        Language.allCases.forEach { language in
            let action = UIAlertAction(title: language.title, style: .default) { [weak self] _ in
                ...
                case .target:
                    self?.translateManager.targetLanuguage = language
